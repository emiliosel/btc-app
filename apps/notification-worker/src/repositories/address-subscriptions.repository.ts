...
export class AddressSubscriptionsRepository {
  ...
  constructor(private readonly connection: MongoClient) {
    this.collection = connection
      .db("btc")
      .collection<AddressSubscriptionEntity>("AddressSubscription");
  }

  async init() {
    this.collection.createIndexes([{ key: { address: 1 } }]);
  }
  ...
  async findSubscriptions(address: string) {
    return this.collection.find({ address }).toArray();
  }
}